...
const getCurrentUserId = () => {
  ...
  console.log('房东端当前用户ID:', currentUserId.value)
}
...
const loadConversations = async () => {
  ...
  try {
    ...
    console.log('房东端会话列表原始数据:', res)
    conversations.value = res.map(conv => ({
      id: conv.id,
      username: conv.otherName || (conv.userId === 0 ? '系统通知' : '用户'),
    ...
    console.log('房东端处理后的会话列表:', conversations.value)
  } catch (error) {
...
const selectConversation = async (item) => {
  ...
  try {
    ...
    if (res.success) {
      ...
      console.log('处理后的消息:', activeMessages.value)
      otherUserInfo.value = res.otherUser
      scrollBottom()
